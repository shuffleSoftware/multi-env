...
!(function(t, e) {
  ...
  const Q = {leftCallback: null, rightCallback: null, endCallback: null};
  const X = {
  ...
  const J = 'left';
  const Z = 'right';
  const tt = 'slid.bs.carousel';
  const et = 'carousel';
  const it = 'active';
  const nt = {ArrowLeft: Z, ArrowRight: J};
  const st = {
    interval: 5e3,
    keyboard: !0,
    pause: 'hover',
  ...
  const lt = 'collapse';
  const ct = 'collapsing';
  const ht = '[data-bs-toggle="collapse"]';
  const dt = {toggle: !0, parent: null};
  const ut = {toggle: 'boolean', parent: '(null|element)'};
  class ft extends F {
    constructor(t, e) {
      ...
      for (const t of n) {
        const e = i(t);
        const n = K.find(e).filter((t) => t === this._element);
  ...
  const Tt = 'popper';
  const Ct = 'reference';
  const Ot = vt.reduce(function(t, e) {
    return t.concat([e + '-' + yt, e + '-' + wt]);
  }, []);
  const xt = [].concat(vt, [bt]).reduce(function(t, e) {
  ...
  const It = 'main';
  const Nt = 'afterMain';
  const Pt = 'beforeWrite';
  const Mt = 'write';
  const jt = 'afterWrite';
  const Ht = [kt, Lt, Dt, St, It, Nt, Pt, Mt, jt];
  function $t(t) {
  ...
  const ae = {
    ...
    fn: function(t) {
      ...
      const a = qt(i.placement);
      const l = ne(a);
      const c = [_t, mt].indexOf(a) >= 0 ? 'height' : 'width';
      if (o && r) {
        ...
        const m = ie(o);
        const _ = m ? ('y' === l ? m.clientHeight || 0 : m.clientWidth || 0) : 0;
        const b = p / 2 - g / 2;
        const v = h[u];
        const y = _ - d[c] - h[f];
        const w = _ / 2 - d[c] / 2 + b;
  ...
  const ce = {top: 'auto', right: 'auto', bottom: 'auto', left: 'auto'};
  function he(t) {
    let e;
    const i = t.popper;
    ...
    const p = r.y;
    let g = void 0 === p ? 0 : p;
    const m = 'function' == typeof h ? h({x: f, y: g}) : {x: f, y: g};
    (f = m.x), (g = m.y);
    ...
    let C;
    const O = Object.assign({position: a}, c && ce);
    const x =
        !0 === h ?
          (function(t) {
            const e = t.x;
  ...
  function Ae(t, e) {
    ...
    const o = Wt(n);
    const r = s ? [o].concat(o.visualViewport || [], ye(n) ? n : []) : n;
    const a = e.concat(r);
  ...
  function Oe(t, e) {
    ...
    const g = oe('number' != typeof p ? p : re(p, vt));
    const m = h === Tt ? Ct : Tt;
    const _ = t.rects.popper;
    const b = t.elements[u ? m : h];
    ...
    const y = Xt(t.elements.reference);
    const w = Ce({reference: y, element: _, strategy: 'absolute', placement: s});
    const A = Ee(Object.assign({}, _, w));
    const E = h === Tt ? A : y;
    const T = {
    ...
    if (h === Tt && C) {
      ...
      Object.keys(T).forEach(function(t) {
        const e = [mt, gt].indexOf(t) >= 0 ? 1 : -1;
        const i = [pt, gt].indexOf(t) >= 0 ? 'y' : 'x';
        T[t] += O[i] * e;
      });
    }
    return T;
  ...
  const Se = {
    ...
    fn: function(t) {
      ...
      const r = Oe(e, {elementContext: 'reference'});
      const a = Oe(e, {altBoundary: !0});
      const l = Le(r, n);
      const c = Le(a, s, o);
      const h = De(l);
      const d = De(c);
  ...
  const Ie = {
    ...
    fn: function(t) {
      ...
      const r = xt.reduce(function(t, i) {
        return (
          (t[i] = (function(t, e, i) {
            const n = qt(t);
            const s = [_t, pt].indexOf(n) >= 0 ? -1 : 1;
            const o =
                    'function' == typeof i ?
  ...
  const Pe = {
    ...
    fn: function(t) {
      ...
      const _ = qt(e.placement);
      const b = le(e.placement);
      const v = !b;
      const y = ne(_);
      const w = 'x' === y ? 'y' : 'x';
      ...
      if (A) {
        if (o) {
          ...
          const K = v ?
                ...
                $ + q + R + O.mainAxis;
          const Q = e.elements.arrow && ie(e.elements.arrow);
          const X = Q ? ('y' === y ? Q.clientTop || 0 : Q.clientLeft || 0) : 0;
          const Y = null != (L = null == x ? void 0 : x[y]) ? L : 0;
          const U = N + K - Y;
          const G = se(f ? Kt(P, N + V - Y - X) : P, N, f ? Vt(M, U) : M);
          (A[y] = G), (k[y] = G - N);
        }
        if (a) {
          let J;
  ...
  function Me(t, e, i) {
    ...
    const r =
        ...
        (function(t) {
          ...
          const i = Qt(e.width) / t.offsetWidth || 1;
          const n = Qt(e.height) / t.offsetHeight || 1;
          return 1 !== i || 1 !== n;
        })(e);
    const a = Zt(e);
    const l = Xt(t, r);
  ...
  const He = {placement: 'bottom', modifiers: [], strategy: 'absolute'};
  function $e() {
  ...
  const Be = We();
  const Fe = We({defaultModifiers: [fe, Ne, de, Rt]});
  const ze = We({defaultModifiers: [fe, Ne, de, Rt, Ie, ke, Pe, ae, Se]});
  ...
  const Ve = 'ArrowUp';
  const Ke = 'ArrowDown';
  const Qe = 'click.bs.dropdown.data-api';
  const Xe = 'keydown.bs.dropdown.data-api';
  const Ye = 'show';
  const Ue = '[data-bs-toggle="dropdown"]:not(.disabled):not(:disabled)';
  const Ge = `${Ue}.show`;
  const Je = '.dropdown-menu';
  const Ze = p() ? 'top-end' : 'top-start';
  const ti = p() ? 'top-start' : 'top-end';
  const ei = p() ? 'bottom-end' : 'bottom-start';
  const ii = p() ? 'bottom-start' : 'bottom-end';
  const ni = p() ? 'left-start' : 'right-start';
  const si = p() ? 'right-start' : 'left-start';
  const oi = {
    offset: [0, 2],
  ...
  class ai extends F {
    ...
    static dataApiKeydownHandler(t) {
      ...
      const n = [Ve, Ke].includes(t.key);
      if (!n && !i) return;
      if (e && !i) return;
      t.preventDefault();
      const s = K.findOne(Ue, t.delegateTarget.parentNode);
      const o = ai.getOrCreateInstance(s);
  ...
  const ci = '.sticky-top';
  const hi = 'padding-right';
  const di = 'margin-right';
  class ui {
    constructor() {
  ...
  const fi = 'show';
  const pi = 'mousedown.bs.backdrop';
  const gi = {
  ...
  const wi = {trapElement: 'element', autofocus: 'boolean'};
  class Ai extends B {
    constructor(t) {
      super(),
      (this._config = this._getConfig(t)),
  ...
  const Ti = 'show.bs.modal';
  const Ci = 'modal-open';
  const Oi = 'show';
  const xi = 'modal-static';
  const ki = {backdrop: !0, keyboard: !0, focus: !0};
  const Li = {
  ...
  const Ii = 'showing';
  const Ni = 'hiding';
  const Pi = '.offcanvas.show';
  const Mi = 'hidePrevented.bs.offcanvas';
  const ji = 'hidden.bs.offcanvas';
  const Hi = {backdrop: !0, keyboard: !0, scroll: !1};
  const $i = {
    backdrop: '(boolean|string)',
    keyboard: 'boolean',
    scroll: 'boolean',
  ...
  const Fi = /^(?:(?:https?|mailto|ftp|tel|file|sms):|[^#&/:?]*(?:[#/?]|$))/i;
  const zi =
  ...
  const ln = {...rn.DefaultType, content: '(null|string|element|function)'};
  class cn extends rn {
    static get Default() {
      return an;
    }
  ...
  const Tn = `.nav-link:not(.dropdown-toggle), .list-group-item:not(.dropdown-toggle), [role="tab"]:not(.dropdown-toggle), ${En}`;
  class Cn extends F {
    constructor(t) {
      super(t),
  ...
  const xn = 'show';
  const kn = 'showing';
  const Ln = {animation: 'boolean', autohide: 'boolean', delay: 'number'};
  const Dn = {animation: !0, autohide: !0, delay: 5e3};
  class Sn extends F {
    constructor(t, e) {
      super(t, e),
      (this._timeout = null),
