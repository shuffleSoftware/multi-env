...
!(function(t, e) {
  ...
  const U = {leftCallback: null, rightCallback: null, endCallback: null};
  const G = {
  ...
  const et = 'left';
  const it = 'right';
  const st = 'slid.bs.carousel';
  const nt = 'carousel';
  const ot = 'active';
  const rt = {ArrowLeft: it, ArrowRight: et};
  const at = {
    interval: 5e3,
    keyboard: !0,
    pause: 'hover',
  ...
  const dt = 'collapse';
  const ut = 'collapsing';
  const _t = '[data-bs-toggle="collapse"]';
  const gt = {toggle: !0, parent: null};
  const ft = {toggle: 'boolean', parent: '(null|element)'};
  class pt extends R {
    constructor(t, e) {
      ...
      for (const t of i) {
        const e = o(t);
        const i = Y.find(e).filter((t) => t === this._element);
  ...
  const bt = 'ArrowUp';
  const vt = 'ArrowDown';
  const yt = 'click.bs.dropdown.data-api';
  const wt = 'keydown.bs.dropdown.data-api';
  const At = 'show';
  const Tt = '[data-bs-toggle="dropdown"]:not(.disabled):not(:disabled)';
  const Et = `${Tt}.show`;
  const Ct = '.dropdown-menu';
  const kt = m() ? 'top-end' : 'top-start';
  const Lt = m() ? 'top-start' : 'top-end';
  const Ot = m() ? 'bottom-end' : 'bottom-start';
  const It = m() ? 'bottom-start' : 'bottom-end';
  const St = m() ? 'left-start' : 'right-start';
  const Dt = m() ? 'right-start' : 'left-start';
  const Nt = {
    offset: [0, 2],
  ...
  class xt extends R {
    ...
    static dataApiKeydownHandler(t) {
      ...
      const s = [bt, vt].includes(t.key);
      if (!s && !i) return;
      if (e && !i) return;
      t.preventDefault();
      const n = Y.findOne(Tt, t.delegateTarget.parentNode);
      const o = xt.getOrCreateInstance(n);
  ...
  const jt = '.sticky-top';
  const $t = 'padding-right';
  const Ft = 'margin-right';
  class Ht {
    constructor() {
  ...
  const zt = 'show';
  const qt = 'mousedown.bs.backdrop';
  const Bt = {
  ...
  const Kt = 'backward';
  const Qt = {trapElement: null, autofocus: !0};
  const Xt = {trapElement: 'element', autofocus: 'boolean'};
  class Yt extends W {
    constructor(t) {
      super(),
      (this._config = this._getConfig(t)),
  ...
  const Gt = 'show.bs.modal';
  const Jt = 'modal-open';
  const Zt = 'show';
  const te = 'modal-static';
  ...
  const oe = 'showing';
  const re = 'hiding';
  const ae = '.offcanvas.show';
  const le = 'hidePrevented.bs.offcanvas';
  const ce = 'hidden.bs.offcanvas';
  const he = {backdrop: !0, keyboard: !0, scroll: !1};
  const de = {
    backdrop: '(boolean|string)',
    keyboard: 'boolean',
    scroll: 'boolean',
  ...
  const ge = /^(?:(?:https?|mailto|ftp|tel|file|sms):|[^#&/:?]*(?:[#/?]|$))/i;
  const fe =
  ...
  const xe = {...Ne.DefaultType, content: '(null|string|element|function)'};
  class Me extends Ne {
    static get Default() {
      return Pe;
    }
  ...
  const Ue = `.nav-link:not(.dropdown-toggle), .list-group-item:not(.dropdown-toggle), [role="tab"]:not(.dropdown-toggle), ${Ye}`;
  class Ge extends R {
    constructor(t) {
      super(t),
    ...
    show() {
      ...
      const i = e ? $.trigger(e, 'hide.bs.tab', {relatedTarget: t}) : null;
